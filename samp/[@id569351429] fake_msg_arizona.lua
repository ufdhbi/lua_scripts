...
function main()
    ...
    sampRegisterChatCommand("fsay", function(p) 
        if p then
            local id, text = p:match("(%d+) (.+)")
            if id then
                sampAddChatMessage(u8d("{FFFFFF}%s[%s]:{B7AFAF} %s"):format(sampGetPlayerNickname(id), id, text), -1)
                emul_rpc("onApplyPlayerAnimation", {tonumber(id), "PED", "IDLE_CHAT", false, false, true, true, 120862771})
                emul_rpc("onPlayerChatBubble", {tonumber(id), -1, 15, 10000, text})
            else
